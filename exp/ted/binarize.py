...
def create_dictionary():
    ...
    for input_file, base_filename in zip(args.input, base_filenames):
        ...
        if os.path.isfile(count_filename) and not args.overwrite:
            ...
            if  'test' in base_filename:
                ...
                logger.warning('Do not use counts from test set')
            elif 'dev' in base_filename:
                update_global_counter = False
                logger.warning('Do not use counts from dev set')
            else:
    ...
    if args.vocab is not None:
        vocab_size = len([v for v in combined_counter.values() if v >= args.vocab])
        vocab_count = combined_counter.most_common(vocab_size)
...
def binarize():
    ...
    for input_file, base_filename, sentence_count in \
        ...
        for sentence_count, sentence in enumerate(input_file):
            ...
            binarized_sentence = [vocab.get(word, 0) for word in words]
